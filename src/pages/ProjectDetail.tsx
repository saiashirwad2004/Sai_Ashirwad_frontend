    ...
    <PageTransition>
      <div className="pt-24 pb-6 relative z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link to="/projects" className="btn-ghost text-[13px]">
            ...
            <div className="lg:col-span-5 order-2 lg:order-1 pt-4">
              <FadeIn>
                ...
                <div className="flex flex-wrap gap-2 mb-10">
                  {(project.tags || []).map(t => (
                    <span key={t} className="flex items-center gap-1.5 tag-chip px-3 py-1.5 text-[13px]">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="btn-ghost group">
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="prose dark:prose-invert max-w-none text-muted-foreground text-sm font-medium">
                ...
                <div className="whitespace-pre-line leading-relaxed glass-card p-6 sm:p-8">
                  {project.longDescription}
                </div>
              </div>
            </FadeIn>
              ...
              {related.map((rp, i) => (
                <motion.div key={rp._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  <Link to={`/projects/${rp.slug}`} className="group block h-full">
                    <div className="relative overflow-hidden glass-card hover:-translate-y-1 transition-all duration-500 h-full flex flex-col">
                      <div className="aspect-[16/9] overflow-hidden bg-muted/50 relative p-2">
                        <div className="w-full h-full rounded-[1.2rem] overflow-hidden border border-border/50 relative">
                          {rp.image?.url ? <img src={rp.image.url} alt={rp.title} className="w-full h-full object-cover group-hover:scale-105 group-hover:rotate-1 transition-transform duration-700 ease-out" /> : <div className="w-full h-full flex items-center justify-center text-muted-foreground bg-card/50"><FolderKanban className="w-10 h-10 mb-2 opacity-20" /></div>}
