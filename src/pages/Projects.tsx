            ...
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map((c) => (
                <button key={c} onClick={() => setActiveCategory(c)}
                  className={`px-5 py-2 rounded-[0.8rem] text-[13px] font-bold transition-all duration-300 shadow-sm ${activeCategory === c ? 'bg-primary text-primary-foreground shadow-primary/30 scale-105' : 'bg-card border border-border text-muted-foreground hover:bg-muted hover:text-foreground hover:border-primary/50'}`}>
                  {c}
                </button>
              ))}
            </div>
              ...
              <motion.div key={activeCategory} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}
                ...
                {filteredProjects.map((p, i) => (
                  <motion.div key={p._id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1, duration: 0.4 }}>
                    <div className="group h-full flex flex-col rounded-[1.5rem] bg-card border border-border/50 hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/20 transition-all duration-500 overflow-hidden relative">
                      {/* Inner Glow Hack */}
                      ...
                      <div className="relative aspect-[4/3] overflow-hidden bg-muted/30 p-2">
                        <div className="w-full h-full rounded-[1.2rem] overflow-hidden relative border border-border/50">
                          {p.image?.url ? <img src={p.image.url} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-700 ease-out" />
                            : <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground bg-card"><FolderKanban className="w-10 h-10 mb-3 opacity-20" />No Image</div>}
                          ...
                          <div className="absolute inset-0 bg-background/60 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center gap-4">
                            <div className="flex items-center gap-3 translate-y-6 group-hover:translate-y-0 transition-all duration-500">
                              {p.github && <a href={p.github} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="p-3 rounded-full bg-card shadow-xl border border-border text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary hover:scale-110 transition-all duration-300"><Github className="w-5 h-5" /></a>}
                              {p.live && <a href={p.live} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="p-3 rounded-full bg-card shadow-xl border border-border text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary hover:scale-110 transition-all duration-300"><ExternalLink className="w-5 h-5" /></a>}
                            </div>
